...
class MSELoss_ADPIT(object):
    ...
    def _each_calc(self, output, target):
        # output/target: [batch, frames, axis, classes]
        loss = self._each_loss(output, target).mean(dim=2)  # [batch, frames, classes]
...
class FrequencyAttention(nn.Module):
    ...
    def forward(self, x):
        # x: [batch, channels, time, frequency]
        freq_descriptor = x.mean(dim=(1, 2), keepdim=False)  # [batch, frequency]
        freq_descriptor = freq_descriptor.unsqueeze(1)       # [batch, 1, frequency]
        freq_weights = self.sigmoid(self.conv(freq_descriptor))
        freq_weights = freq_weights.unsqueeze(2)             # [batch, 1, 1, frequency]
...
class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1)):
        ...
        self.freq_att = FrequencyAttention(kernel_size=15)
