...
class DataGenerator(object):
    def __init__(self, params, split=1, shuffle=True, per_file=False, is_eval=False):
        ...
        self._foa_channel_mode = params.get('foa_channel_mode', 'all')
    ...
    def _apply_foa_channel_ablation(self, temp_feat):
        """
        Applies FOA channel ablation without changing feature dimensions.
        
        Feature layout for FOA:
        channels 0-3: log-mel W, X, Y, Z
        channels 4-6: FOA intensity vectors
        """
        if self._foa_channel_mode == "all":
            return temp_feat

        temp_feat = temp_feat.reshape(temp_feat.shape[0], self._nb_ch, self._nb_mel_bins)

        if self._foa_channel_mode == "w_only":
            temp_feat[:, 1:, :] = 0.0

        elif self._foa_channel_mode == "xyz_only":
            temp_feat[:, 0, :] = 0.0
            temp_feat[:, 4:, :] = 0.0

        elif self._foa_channel_mode == "wxyz_only":
            temp_feat[:, 4:, :] = 0.0

        elif self._foa_channel_mode == "iv_only":
            temp_feat[:, :4, :] = 0.0

        else:
            raise ValueError(f"Unknown foa_channel_mode: {self._foa_channel_mode}")

        return temp_feat.reshape(temp_feat.shape[0], -1)
        ...
        if self._is_eval:
            for i in range(self._nb_total_batches):
                ...
                while (len(self._circ_buf_feat) < self._feature_batch_seq_len or (hasattr(self, '_circ_buf_vid_feat') and hasattr(self, '_vid_feature_batch_seq_len') and len(self._circ_buf_vid_feat) < self._vid_feature_batch_seq_len)):
                    ...
                    temp_feat = self._apply_foa_channel_ablation(temp_feat)
