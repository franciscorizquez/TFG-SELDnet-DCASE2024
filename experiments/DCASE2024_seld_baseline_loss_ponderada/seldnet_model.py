...
class WeightedMSELoss_ADPIT(object):  #AÑADIDA
    def __init__(self, dist_weight=0.5):
        super().__init__()
    ...
    def _each_calc(self, output, target):
        ...
        # output/target shape después del reshape:
        # [batch, frames, 12, classes]
        # porque son 3 tracks * 4 valores (x,y,z,dist)
        weights = torch.ones_like(loss)
        weights[:, :, 3::4, :] = self._dist_weight
    ...
    def __call__(self, output, target):
        ...
        output = output.reshape(
            output.shape[0],
            output.shape[1],
            target_A0A0A0.shape[2],
            target_A0A0A0.shape[3]
        )
