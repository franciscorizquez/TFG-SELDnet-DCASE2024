...
class FeatureClass:
    ...
    def _get_spectrogram_for_file(self, audio_filename):
        ...
        return audio_spec
    def _scale_distance_label(self, dist_value):  #AÑADIDO (todas las líneas que llamen a la func han sido mod. por consecuencia)
        """
        Convierte la distancia original a metros y aplica log-scaling.

        Entrada:
            dist_value: distancia original del metadata, normalmente en centímetros.

        Salida:
            distancia escalada para entrenamiento.
        """
        dist_m = dist_value / 100.
        return np.log1p(dist_m)
        ...
        for frame_ind, active_event_list in _desc_file.items():
            if frame_ind < _nb_label_frames:
                for active_event in active_event_list:
                    ...
                    dist_label[frame_ind, active_event[0]] = self._scale_distance_label(active_event[5]) 
                ...
                for i, active_event in enumerate(active_event_list):
                    ...
                    if i == len(active_event_list) - 1:  # if the last
                        if len(active_event_list_per_class) == 1:  # if no ov from the same class
                            ...
                            dist_label[frame_ind, 0, active_event_a0[0]] = self._scale_distance_label(active_event_a0[5]) 
                            ...
                            dist_label[frame_ind, 1, active_event_b0[0]] = self._scale_distance_label(active_event_b0[5])
                            # --b1--
                            ...
                            dist_label[frame_ind, 2, active_event_b1[0]] = self._scale_distance_label(active_event_b1[5])
                            ...
                            dist_label[frame_ind, 3, active_event_c0[0]] = self._scale_distance_label(active_event_c0[5])
                            # ----c1
                            ...
                            dist_label[frame_ind, 4, active_event_c1[0]] = self._scale_distance_label(active_event_c1[5])
                            # ----c2
                            ...
                            dist_label[frame_ind, 5, active_event_c2[0]] = self._scale_distance_label(active_event_c2[5])
